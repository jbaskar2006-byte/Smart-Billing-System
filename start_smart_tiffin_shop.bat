...
start "Smart Tiffin Shop - Backend Server" cmd /k "cd /d %~dp0smart-tiffin-shop\backend && server.exe"

echo Waiting for backend server initialization...
timeout /t 3 /nobreak >nul

echo [2/2] Launching React Vite Frontend App on http://localhost:5173 ...
start "Smart Tiffin Shop - Frontend App" cmd /k "cd /d %~dp0smart-tiffin-shop\frontend && npm run dev"
